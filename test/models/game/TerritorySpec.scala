...
class TerritorySpec extends PlaySpec {
  ...
  val sut = Territory(Set(Hex(1, 1)), Player(1))
  ...
  "potentialNeighbors" must {
    ...
    "work for another hex" in {
      ...
      Territory(Set(Hex(0, 1)), Player(1)).potentialNeighbors mustBe Set((0, 0), (-1, 0), (-1, 1), (0, 2), (1, 1), (1, 0)).map(Hex.tupled.apply)
    }

    "work for two hex" in {
      val expected = Set((0, 0), (-1, 0), (-1, 1), (0, 2), (1, 2), (2, 2), (2, 1), (1, 0)).map(Hex.tupled.apply)
      val territory = Territory(Set(Hex(0, 1), Hex(1, 1)), Player(1))
  ...
  "isTouching" must {
    "find the touching hex" when {
      Seq(
        ...
        s"it is to the $direction" in {
          ...
          assert(sut.isTouching(Territory(Set(touchingHex), Player(2))))
        }
      }
    }
    ...
    "be false when divided by a hex" in {
      val thisT = Territory(Set(Hex(1, 0)), Player(1))
      val otherT = Territory(Set(Hex(1, 2)), Player(2))
    ...
    "work for multiples" when {
      "it contains more than 1" in {
        val thisT = Territory(Set(Hex(1, 1), Hex(1, 2)), Player(1))
        val otherT = Territory(Set(Hex(1, 3), Hex(1, 4)), Player(2))

        assert(thisT.isTouching(otherT))
      }
      "when it touches more than once" in {
        val thisT = Territory(Set(Hex(0, 0), Hex(0, 1)), Player(1))
        val otherT = Territory(Set(Hex(1, 0)), Player(2))
  ...
  "neighbors" must {
    "return the neighboring players" when {
      "complex" in {
        val territory1 = Territory(Set(Hex(0, 0), Hex(0, 1), Hex(1, 0)), Player(1))
        val territory2 = Territory(Set(Hex(0, 2), Hex(1,1), Hex(2, 1)), Player(2))
        val territory3 = Territory(Set(Hex(1, 2), Hex(2, 2)), Player(3))
        val all = Set(territory1, territory2, territory3)

        withClue("1 should be neighbors with 2") {
          territory1.neighbors(all).map(_.player.number) mustBe Set(2)
        }
        withClue("2 should be neighbors with 1 and 3") {
          territory2.neighbors(all).map(_.player.number) mustBe Set(1, 3)
        }
        withClue("3 should be neighbors with 2") {
          territory3.neighbors(all).map(_.player.number) mustBe Set(2)
        }
      }
    }
  }
  ...
  "attackable" must {
    "return the neighboring enemy players" when {
      "complex" in {
        val territory1 = Territory(Set(Hex(0, 0), Hex(0, 1), Hex(1, 0)), Player(1))
        val territory2 = Territory(Set(Hex(0, 2), Hex(1,1), Hex(2, 1)), Player(2))
        val territory3 = Territory(Set(Hex(1, 2), Hex(2, 2)), Player(1))
        val all = Set(territory1, territory2, territory3)
